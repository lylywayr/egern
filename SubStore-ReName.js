 ...
 * 格式：https://raw.githubusercontent.com/你的用户名/仓库名/rename.js#参数1=值&参数2=值...
 * 
 * 注意：布尔类型参数（如 flag、clear）只有显式传递 =1 或 =true 时才启用。
 ...
 *   out=zh|en|quan|flag        输出格式（默认与 in 相同，若 in 未指定则默认 zh）
 *   fgf=分隔符                 前缀/国旗与名称间的分隔符（默认空格，需 URL 编码）
 ...
 * 
 * 【关键词保留与替换】
 *   blkey=关键词1+关键词2>新名  保留指定关键词，多个用 +，可用 > 替换（如 IPLC>专线）
 *   blgd=1                     保留固定格式标识（IPLC、IEPL、核心、边缘等）
 *   bl=1                       保留倍率标识（如 x2、3×）
 *   blnx=1                     只保留高倍率（>1x）节点
 *   nx=1                       保留 1x 或无倍率节点（与 bl 配合）
 *   blpx=1                     对保留的倍率标识分组排序（特殊标识优先）
 * 
 * 【过滤与清理】
 *   clear=1                    清理含乱名关键词（如“套餐”“到期”）的节点
 *   key=1                      启用额外过滤规则（内部正则）
 *   nm=1                       保留未匹配到地区的节点（只加前缀）
 * 
 * 【其他功能】
 *   flag=1                     在节点名中添加国旗图标
...
// ---------- 辅助函数：严格解析布尔值 ----------
// 只有显式传入 "1"、1、true 或 "true" 时才返回 true
