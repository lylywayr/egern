/**
 * PingMe 获取Cookie脚本（Egern 开关版）
 * 开关变量：ENABLE_COOKIE，通过模块 env_schema 传入
 * 原始通用脚本来自 fmz200，完整保留所有应用逻辑
 */
...
(() => {
    let enable = true;
    try {
        if (typeof ctx !== 'undefined' && ctx.env && ctx.env.ENABLE_COOKIE === 'false') enable = false;
        if (typeof $env !== 'undefined' && $env.ENABLE_COOKIE === 'false') enable = false;
    } catch (e) {}
    if (!enable) {
